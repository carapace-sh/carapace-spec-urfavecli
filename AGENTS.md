...
`carapace-spec-urfavecli` is a Go **library** (no `main` package, no binary) that generates [carapace-spec](https://github.com/carapace-sh/carapace-spec) YAML from [urfave/cli/v2](https://github.com/urfave/cli) applications. It scrapes an existing `*cli.App` command/flag tree and emits a `command.Command` spec consumable by the carapace completion engine.
...
- **`spec.go`** — Public API and the scraping engine.
  - `Register(app *cli.App)` appends a hidden `_carapace` command (with a `spec` subcommand) to `app.Commands`. The subcommand's action marshals `Command(app)` to YAML via `gopkg.in/yaml.v2` and prints it.
  - `Command(app *cli.App) command.Command` is the entry point for programmatic use. It wraps the app's top-level fields into a synthetic `*cli.Command` and delegates to `scrape`.
...
```
*cli.App ──Command()──> synthetic *cli.Command ──scrape()──> command.Command ──yaml.Marshal──> YAML stdout
                              │                                   │
                              ├── app.Flags                        ├── Flags (via AddFlag)
                              ├── app.Commands                     ├── Commands (recursive scrape)
                              └── app.Usage                        └── Completion.Flag (file completion)
```
...
`TakesFile()` uses a **type switch** over the concrete flag types (`*cli.GenericFlag`, `*cli.StringFlag`, `*cli.StringSliceFlag`, `*cli.PathFlag`) and reads each one's `TakesFile` bool field. New urfave/cli flag types that carry a `TakesFile` field must be added here explicitly — there is no interface to satisfy.
...
`flag.Default()` populates the `Default` field on `command.Flag` (carapace-spec v1.8.0+), which triggers extended YAML notation and is consumed by pflag's `Value.Set()` at registration time. The value comes from `DocGenerationFlag.GetDefaultText()`, which:

- Returns the user's explicit `DefaultText` as-is (no quoting)
- Otherwise returns the `defaultValue` field (the original default saved during `Apply`, immune to env-var overrides)

**Critical gotcha**: `StringFlag`, `PathFlag`, and `StringSliceFlag` wrap their defaults in display quoting (`fmt.Sprintf("%q", val)` or `strconv.Quote` per element) via `GetDefaultText`. Passing quoted strings to pflag's `Value.Set()` would embed literal quote characters in the value. `flag.Default()` calls `strconv.Unquote()` on the result for these types to strip the wrapping quotes. For `StringSliceFlag`, each comma-separated element is unquoted individually. Other flag types (`IntFlag`, `DurationFlag`, `Float64Flag`, etc.) don't apply `%q` and are passed through as-is.
...
- `github.com/urfave/cli/v2 v2.27.7` — the framework being scraped.
